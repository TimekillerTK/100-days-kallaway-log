...
**Link to work:** [YahtzeeScoring](https://github.com/TimekillerTK/DailyProgrammer/commit/dfc5e4c418fe1ccee2ef4f513b8c15a41e5965aa)
...
### Day 53: 04/11/2020 - Wednesday

**Today's Progress:** Not much, wondering how to approach concurrency/parallelism

**Thoughts:** I tried my hand at one of the bonus challenges, and found that iterating through all of the words in `list.txt` takes a very long time (4minutes+). I tried using `Foreach-Object -Parallel`, however I learned that you're not able to pass outside functions into `ForEach-Parallel`. The reason why is because `ForEach-Object -Parallel` runs each loop iteration in a runspace initialized to its default state. There is a proposed change to this with a switch like `-UseCurrentState`, but this will have a performance penalty, which I'd not want to take advantage of anyway.

One solution is to insert the function directly into the loop, another solution instead is as follows:
```powershell
...
Tomorrow, I'll try to solve the challenge without worrying about this for now, as it sent me down a rabbithole of complexity. Let it be slow. For now, it needs to work :)

**Link to work:**  [MorseCode](https://github.com/TimekillerTK/DailyProgrammer/commit/b374d079490452df906707d9f8c5a098d4ce7b76)